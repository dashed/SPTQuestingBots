...
public sealed class SquadRegistry
{
    ...
    public SquadEntity GetOrCreate(int bsgGroupId, int strategyCount, int targetMembers)
    {
        if (_bsgGroupToSquadId.TryGetValue(bsgGroupId, out var squadId))
        {
            ...
            LoggingController.LogWarning("[SquadRegistry] Stale BSG group mapping for groupId=" + bsgGroupId + " -> squadId=" + squadId);
    ...
    public bool Remove(SquadEntity squad)
    {
        ...
        LoggingController.LogInfo(
            "[SquadRegistry] Removing squad " + squad.Id + " (members=" + squad.Members.Count + ", remaining=" + (_squads.Count - 1) + ")"
        );
    ...
    public void RemoveMember(SquadEntity squad, BotEntity member)
    {
        ...
        LoggingController.LogInfo(
            "[SquadRegistry] Removing entity " + member.Id + " from squad " + squad.Id + " (remaining=" + (squad.Members.Count - 1) + ")"
        );
