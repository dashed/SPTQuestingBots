...
public static class SquadLootCoordinator
{
    ...
    public static void ShareScanResults(SquadEntity squad, LootScanResult[] bossResults, int bossResultCount)
    {
        ...
        {
            LoggingController.LogDebug(
                "[SquadLootCoordinator] Shared " + shareCount + " loot targets with squad" + " (from " + bossResultCount + " boss results)"
            );
        }
    }
    ...
    /// </summary>
    public static int PickSharedTargetForFollower(SquadEntity squad, int followerEntityId, int bossLootTargetId, LootClaimRegistry claims)
    {
        int bestIndex = -1;
        float bestValue = -1f;
