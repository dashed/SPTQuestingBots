...
public class HoldTacticalPositionTaskTests
{
    ...
    {
        var task = new HoldTacticalPositionTask();
        var entity = CreateFollowerWithTacticalPosition(0, 0, 0, 1, 0, 0, aggression: 0.5f, raidTime: 0.5f);

        task.ScoreEntity(0, entity);

        float rawScore = HoldTacticalPositionTask.Score(entity);
        float modifier = ScoringModifiers.CombinedModifier(0.5f, 0.5f, 0f, BotActionTypeId.HoldPosition);
